...
# Cloud init of salt minion and master running on the same instance
# This only contains the parts that doesn't require any secrets to prevent
# leaking them if anyone gets access to the cloud-init user-data later.
...
# Update the repo to get updated keys and archives, then install https
# transport for apt before adding the salt repo using https, and gnupg for
# apt-key
apt-get update
apt_get_noninteractive install apt-transport-https gnupg2 git
