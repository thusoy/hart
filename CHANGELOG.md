...
- `-S` or `--script` can now also be passed to create-minion if there's some
  setup to be done that is easier to do in a script than through salt.
...
## Fixed
- Improved resiliency of the default minion config for a saltmaster failover.
...
## Fixed
- GCP images failing to boot due to picking the wrong architecture for the boot image.
